    ...
    for (const param of params) {
      ...
      if (param.schema) {
        ...
        if (param.schema.enum)
          out += `- **Allowed Values:** ${param.schema.enum.join(", ")}\n`;
        if (param.schema.minimum !== undefined)
          out += `- **Minimum:** ${param.schema.minimum}\n`;
        if (param.schema.maximum !== undefined)
          out += `- **Maximum:** ${param.schema.maximum}\n`;
        if (param.schema.pattern)
          out += `- **Pattern:** \`${param.schema.pattern}\`\n`;
        if (param.schema.example !== undefined)
          out += `- **Example:** \`${param.schema.example}\`\n`;
      }
...
export function formatQueryParameters(parameters: any[]): string {
  if (!parameters || parameters.length === 0) return "";
  const queryParams = parameters.filter((param) => param.in === "query");
  if (queryParams.length === 0) return "";
  let out = "## Query Parameters\n\n";
...
export function formatRequestBody(requestBody: any): string {
  ...
  if (requestBody.description)
    out += `- **Description:** ${requestBody.description}\n`;
  if (requestBody.schema) {
    out += `\n**Schema:**\n`;
    out += "```json\n";
    out += formatSchema(requestBody.schema) + "\n";
    out += "```\n";
  }
  return out.trim() + "\n";
}
      ...
      if (param.required) {
        ...
        if (param.schema) {
          if (param.schema.type === "integer") exampleValue = "123";
            ...
            exampleValue = flattedStringify(safeClone(param.schema.example));
